...
if q_closeCont == 1:
    ...
    for ch in channels:
        ...
        if q_autom == 1:     
            ...
            stack_closed = fcCont.automCloseStackContours(myStack = stack, ch = ch, slices = (slc_first_close,slc_last_close), 
        ...
        if q_manual == 1: 
            ...
            stack_closed, slc_first_o = fcCont.manuallyCloseContours (stack_closed = stack_closed, 
                                                ...
                                                n_rows = 6, chStr = ch)
            fcCont.showGridContours(myStack = stack_closed, slices = (slc_first,slc_last), n_rows = 6)
        ...
        if q_infOutf == 1: 
            for region in ['Inflow', 'Outflow']:
                ...
                if q_region == 1:
                    ...
                    fcCont.showGridContours(myStack = stack_closed, slices = (slc_reg_first,slc_reg_last), n_rows = 6)
                    # fcCont.showContours(myStack = stack_closed, slices = (slc_reg_first,slc_reg_last), minLenContour = 250, 
                    #                           plotEvery = 5, figureSize = (5,5), plotshow= True)
            initial_runA = False
...
#                     plotEvery = 1, figureSize = (5,5), plotshow= True)
#%% ii. SELECT CONTOURS - what if the contours have already been saved and are being processed in the same console
if q_closeCont == 0:
    # Get channel(s)
    channels = fcCont.selectChannel()
