...
if init:
    ...
    #%% Import dafaframe
    sns.set_style("white")
    colors_pts = sns.hls_palette(8, h=.5, l=.4)

    df_classCJTh = fcBasics.loadDF(filename = filename, file = 'df_cjThNmyocIntBall', dir_results = dir_results)

    #%%
    #Organise DFs
    df_ALD = df_classCJTh[(df_classCJTh['AtrVent'] == 'atrium') & (df_classCJTh['LeftRight'] == 'left')
                          & (df_classCJTh['DorsVent'] == 'dorsal')]
    df_ALV = df_classCJTh[(df_classCJTh['AtrVent'] == 'atrium') & (df_classCJTh['LeftRight'] == 'left')
                          & (df_classCJTh['DorsVent'] == 'ventral')]
    df_ARD = df_classCJTh[(df_classCJTh['AtrVent'] == 'atrium') & (df_classCJTh['LeftRight'] == 'right')
                          & (df_classCJTh['DorsVent'] == 'dorsal')]
    df_ARV = df_classCJTh[(df_classCJTh['AtrVent'] == 'atrium') & (df_classCJTh['LeftRight'] == 'right')
                          & (df_classCJTh['DorsVent'] == 'ventral')]

    df_VLD = df_classCJTh[(df_classCJTh['AtrVent'] == 'ventricle') & (df_classCJTh['LeftRight'] == 'left')
                          & (df_classCJTh['DorsVent'] == 'dorsal')]
    df_VLV = df_classCJTh[(df_classCJTh['AtrVent'] == 'ventricle') & (df_classCJTh['LeftRight'] == 'left')
                          & (df_classCJTh['DorsVent'] == 'ventral')]
    df_VRD =  df_classCJTh[(df_classCJTh['AtrVent'] == 'ventricle') & (df_classCJTh['LeftRight'] == 'right')
                          & (df_classCJTh['DorsVent'] == 'dorsal')]
    df_VRV = df_classCJTh[(df_classCJTh['AtrVent'] == 'ventricle') & (df_classCJTh['LeftRight'] == 'right')
                          & (df_classCJTh['DorsVent'] == 'ventral')]
    #Add colum with this classification for plotting easier

    df_classALL = [df_ALD, df_ALV, df_ARD, df_ARV, df_VLD, df_VLV, df_VRD, df_VRV]
    class_name = ['Atrium-Left_Dorsal','Atrium-Left_Ventral','Atrium-Right_Dorsal','Atrium-Right_Ventral',
                  'Ventricle-Left_Dorsal','Ventricle-Left_Ventral','Ventricle-Right_Dorsal','Ventricle-Right_Ventral']

    for num, sp_df in enumerate(df_classALL):
        n = len(sp_df)
        name = [class_name[num]]*n
        sp_df['Class'] = name
    ...
    for k, x, label in zip(count(), ['myoc_intBall'], ['Myoc.Int Ballooning [um]']):
        ...
        # Plot All
        fig4 = sns.kdeplot(
            data=df_concat, x=x, hue="Class",
            fill=True, common_norm=False, palette=colors_pts,
