...
if init: 
    ...
    m_atr, m_vent, pl_Chambers, dict_planes, dict_pts, num_pt = fcMeshes.getChamberMeshes(filename = filename, s3s2cut = s3s2cut, names2cut = ['Myoc','Endo', 'CJ', 'Ext.Myoc', 'Int.Endo'],
                                                        kspl_CL = kspl_CL[0], mesh2cut = m_myoc, resolution = res, dict_planes = dict_planes, dict_pts = dict_pts)
    ...
    if plot: 
        ...
        vp.show(m_atr[4], m_vent[4], pl_Chambers.alpha(0.05), at = 5, azimuth = azimuth, interactive=True)
    ...
    # Cardiac Jelly
    cj_thickness, m_cjTh, [cjmin, cjmax] = fcMeshes.getDistance2Mesh(filename = filename, m_int = m_cjIn, m_ext = m_cjOut, 
                                                        title = 'Cardiac Jelly Thickness', plotshow = False)
    # Myocardium
    myoc_thickness, m_myocTh, [myocmin, myocmax] = fcMeshes.getDistance2Mesh(filename = filename, m_int = m_myocInt, m_ext = m_myocExt, 
                                                        title = 'Myoc.Thickness', plotshow = False)
    # Endocardium
    endo_thickness, m_endoTh, [endomin, endomax] = fcMeshes.getDistance2Mesh(filename = filename, m_int = m_endoInt, m_ext = m_endoExt,
    ...
    if plot:   
        ...
        vp.show(m_myocExt.alpha(0.01).color(dict_colour['myoc_ext']['colour']), sph_ballonning, at=1)
        vp.show(m_myocInt.alpha(0.01), m_myocExt.alpha(0.01), sph_ballonning, scale_cube, at=2, elevation = elevation, interactive = True)
    ...
    if save: 
        ...
        fcMeshes.saveMultVideos(filename = filename, 
                    ...
                    meshes4video = [m_myoc,m_endo,m_cj,[m_cjTh, m_cjIn],[m_myocTh, m_myocInt],[m_endoTh, m_endoInt],m_myocIntBall.alpha(1),m_myocExtBall.alpha(1)], 
                    rotAngle= df_res.loc[file_num,'ang_Heart'], dir2save = directories[4], plotshow = False)
        
        #OLD VERSION! 
        # print('Saving videos... this might take a while...')
        ...
        # fcMeshes.saveVideo(filename = filename, info = 'cj_thickness', meshes4video = [m_cjTh, m_cjIn], 
        #                     rotAngle = df_res.loc[file_num,'ang_Heart'], dir2save = directories[4])
        # fcMeshes.saveVideo(filename = filename, info = 'myoc_thickness', meshes4video = [m_myocTh, m_myocInt], 
        #                     rotAngle = df_res.loc[file_num,'ang_Heart'], dir2save = directories[4])
        # fcMeshes.saveVideo(filename = filename, info = 'endo_thickness', meshes4video = [m_endoTh, m_endoInt], 
        #                     rotAngle = df_res.loc[file_num,'ang_Heart'], dir2save = directories[4])
        
        # fcMeshes.saveVideo(filename = filename, info = 'myoc_intBall', meshes4video = [m_myocIntBall.alpha(1)], 
        #                     rotAngle = df_res.loc[file_num,'ang_Heart'], dir2save = directories[4])
        # fcMeshes.saveVideo(filename = filename, info = 'myoc_extBall', meshes4video = [m_myocExtBall.alpha(1)], 
        #                     rotAngle = df_res.loc[file_num,'ang_Heart'], dir2save = directories[4])
    ...
    # #%% Rotate everything 
    # rot_objects, first = fcMeshes.rotateAll (objects_all = [meshes_out, kSplinesCuts, sphCuts, kspl_CL, linLines, sph_CL, sph_CL_colour], 
    #                                                                                                 angles = rotAngles, first = first)
